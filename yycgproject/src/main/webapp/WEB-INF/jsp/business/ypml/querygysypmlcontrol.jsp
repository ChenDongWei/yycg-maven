...
<script type="text/javascript">
	...
	function submitgysypmlcontrol() {
		_confirm('您确定要执行提交选中的控制状态吗?', null, function() {
			...
			if (rows.length > 0) {
				...
				jquerySubByFId('gysypmlcontrolForm',
						submitgysypmlcontrol_callback, null);
			} else {
	...
	var columns = [ [
			...
			},
			{
				...
				formatter : function(value, row, index) {
					...
					var input_v = '<input type="text" name="gysypmlControls['
							+ index + '].advice" value="'
							+ (value ? value : '') + '"/>';
			...
			},
			{
				...
				width : 60,
				formatter : function(value, row, index) {
					return '<a href=javascript:ypxxinfo(\'' + row.ypxxid
							+ '\')>查看</a>';
				}
			} ] ];
	//datagrid加载
			...
			<TABLE class="table_search">
				<TBODY>
					...
					<tr>
						...
						<td><select id="ypxxCustom_lb" name="ypxxCustom.lb"
							style="width: 150px">
						...
						<td><select id="gysypmlCustom.control"
							name="gysypmlCustom.control" style="width: 80px">
								<option value="">全部</option>
								<c:forEach items="${controllist}" var="value">
									<option value="${value.dictcode}">${value.info}</option>
								</c:forEach>
						</select> <a id="btn" href="#" onclick="querygysypmlcontrol()"
